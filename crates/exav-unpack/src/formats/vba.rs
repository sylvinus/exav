...
pub(crate) fn decompress(data: &[u8], cap: u64) -> Option<Vec<u8>> {
    ...
    while pos + 2 <= data.len() {
        // The RLE copy tokens can amplify output far beyond the input; bound the
        // decompressed container at the global peak-buffer limit. Each chunk adds
        // ≤ 4096 bytes, so stopping here caps `out` at `cap + 4096`.
...
/// `Doc.*` macro signatures are written against a dump that lowercases code
/// tokens (keywords/identifiers) but leaves string-literal contents intact, so a
/// signature can match a lowercased identifier next to a case-sensitive string
...
/// (string literals preserved) that `Doc.*` macro signatures match; `raw`
...
pub(crate) fn build_artifacts(streams: &[(String, &[u8])], cap: u64) -> Option<(Vec<u8>, Vec<u8>)> {
    ...
    let dir = decompress(dir_raw, cap)?;
    ...
    for m in &info.modules {
        ...
        out.push('\n');
        // Bound the combined artifact buffers by the global peak-buffer limit:
        // many modules, each ≤ `cap`, must not sum without limit.
        if out.len() as u64 >= cap || raw.len() as u64 >= cap {
            break;
        }
    }
...
mod tests {
    ...
    fn decompress_rejects_bad_signature() {
        assert!(decompress(&[0x00, 0x01, 0x02], u64::MAX).is_none());
        assert!(decompress(&[], u64::MAX).is_none());
    }
    ...
    fn decompress_copy_token_run() {
        ...
        assert_eq!(decompress(&buf, u64::MAX).unwrap(), b"aaaaaaaa");
    }
}
