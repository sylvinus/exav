...
fn write_payload<W: Write>(db: &Database, w: &mut W) -> io::Result<()> {
    ...
    {
        // Empty parts, keeping the cache layout identical across features.
...
mod tests {
    ...
    /// The `.pdb`/`.wdb` phishing DB must survive the prebuilt cache round-trip:
    /// a cache-loaded engine applies the same brand-scoping and allow-listing
    /// (incl. the recompiled `X:` regexes) as a fresh `-d <dbdir>` load.
    #[cfg(feature = "phishing")]
    #[test]
    fn phishing_db_round_trips_through_cache() {
        let dir = tempfile::tempdir().unwrap();
