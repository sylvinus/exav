...
fn main() {
    ...
    let eng = eb.build();
    let hit = eng.scan_with_layout(&data, ft, layout.as_ref());
    println!("scan_with_layout => {:?}", hit);
    let mut all = Vec::new();
    eng.scan_all_with_layout(&data, ft, layout.as_ref(), &mut all);
    println!("scan_all_with_layout => {:?}", all);
    let mut lo = Vec::new();
    eng.scan_logical_offsets(&data, ft, layout.as_ref(), &mut lo);
