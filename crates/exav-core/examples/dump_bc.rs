...
fn main() {
    ...
    println!(
        "num_types: {} num_funcs: {}",
        bc.header.num_types, bc.header.num_funcs
    );
    ...
    for (fi, f) in bc.functions.iter().enumerate() {
        println!(
            "--- fn {fi}: args={} ret={} locals={:?} insts={} bb={}",
            ...
            for ins in blk {
                let b = match &ins.body {
                    ...
                    Body::Call { api, func, args } => {
                        format!("CALL api={} func={} args={:?}", api, func, args)
                    }
